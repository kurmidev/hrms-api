...
export class PayrollStructuresController {
  ...
  @ApiOperation({
    ...
    description: 'Creates a named salary structure with earning and deduction components.',
  })
  @ApiResponse({
    status: 201,
    description: 'Payroll structure created',
    type: PayrollStructureResponseDto,
  })
  @ApiResponse({
    status: 400,
    description:
      'Validation error — duplicate component names, no earning component, or invalid values',
  })
  create(
    @CurrentUser('organizationId') organizationId: string,
  ...
  @ApiOperation({
    ...
    description: 'Paginated list with employee and designation counts.',
  })
  ...
  @ApiOperation({
    ...
    description: 'Returns full structure with components and linked designations.',
  })
  @ApiParam({ name: 'id', description: 'Payroll structure UUID' })
  @ApiResponse({
    status: 200,
    description: 'Payroll structure details',
    type: PayrollStructureResponseDto,
  })
  ...
  @ApiOperation({
    ...
    description: 'Updates name, components, or active status.',
  })
  @ApiParam({ name: 'id', description: 'Payroll structure UUID' })
  @ApiResponse({
    status: 200,
    description: 'Payroll structure updated',
    type: PayrollStructureResponseDto,
  })
  ...
  @ApiOperation({
    summary: 'Soft-delete payroll structure',
    description: 'Soft-deletes. Fails if employees are currently assigned.',
  })
  @ApiParam({ name: 'id', description: 'Payroll structure UUID' })
  ...
  @ApiResponse({
    status: 200,
    description: 'Payroll structure restored',
    type: PayrollStructureResponseDto,
  })
  ...
  @RequirePermissions('payroll:update')
  @ApiOperation({
    summary: 'Assign designation to payroll structure',
    description:
      'Links a designation to this payroll structure. One designation can only be linked to one structure at a time.',
  })
  @ApiParam({ name: 'id', description: 'Payroll structure UUID' })
  ...
  assignDesignation(
    ...
    return this.payrollStructuresService.assignDesignation(
      organizationId,
      structureId,
      designationId,
    );
  ...
  unassignDesignation(
    ...
    return this.payrollStructuresService.unassignDesignation(
      organizationId,
      structureId,
      designationId,
    );
