...
export class RolesController {
  ...
  @ApiOperation({
    ...
    description:
      'Creates a custom role with a set of permissions. System roles cannot be created via this API.',
  })
  ...
  create(@CurrentUser('organizationId') organizationId: string, @Body() dto: CreateRoleDto) {
  ...
  @ApiOperation({
    ...
    description:
      'Returns all roles for the organization, including system roles. System roles are listed first.',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description: 'Returns a single role with its full permission list and user count.',
  })
  ...
  @ApiOperation({
    summary: 'Update role',
    description:
      'Updates the name, description, or permissions of a custom role. System roles cannot be modified.',
  })
  @ApiParam({ name: 'id', description: 'Role UUID' })
  @ApiResponse({ status: 200, description: 'Role updated', type: RoleResponseDto })
  @ApiResponse({
    status: 403,
    description: 'System roles cannot be modified or missing permission: role:update',
  })
  @ApiResponse({ status: 404, description: 'Role not found' })
  ...
  @ApiOperation({
    summary: 'Delete role',
    description:
      'Deletes a custom role. Fails if the role has any users currently assigned to it, or if it is a system role.',
  })
  @ApiParam({ name: 'id', description: 'Role UUID' })
  @ApiResponse({
    status: 200,
    ...
    schema: {
      example: {
        success: true,
        data: { deleted: true, message: 'Role "branch_manager" deleted successfully' },
        timestamp: '2024-01-15T10:30:00.000Z',
      },
    },
  })
  ...
  @ApiResponse({
    status: 403,
    description: 'System roles cannot be deleted or missing permission: role:delete',
  })
  @ApiResponse({ status: 404, description: 'Role not found' })
  @ApiResponse({ status: 401, description: 'Not authenticated' })
  ...
  @ApiOperation({
    ...
    description:
      'Assigns a role to a user in the same organization. If the user already has this role, the request is idempotent.',
  })
  @ApiResponse({
    status: 201,
    ...
    schema: {
      example: {
        success: true,
        data: { assigned: true, message: 'Role "hr_manager" assigned to user' },
        timestamp: '2024-01-15T10:30:00.000Z',
      },
    },
  })
  ...
  assignRole(@CurrentUser('organizationId') organizationId: string, @Body() dto: AssignRoleDto) {
  ...
  @ApiResponse({
    ...
    schema: {
      example: {
        success: true,
        data: { removed: true, message: 'Role removed from user' },
        timestamp: '2024-01-15T10:30:00.000Z',
      },
    },
  })
  ...
  @ApiOperation({
    ...
    description:
      'Returns every role assigned to the specified user, including their full permission lists.',
  })
  @ApiParam({ name: 'userId', description: 'User UUID' })
  @ApiResponse({
    status: 200,
