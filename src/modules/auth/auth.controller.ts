...
export class AuthController {
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Reset password via admin-issued token',
    description:
      'Validates a 24-hour reset token (generated by admin via POST /employees/:id/send-password-reset), updates the password, and sends a confirmation email.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Returns all open sessions for the authenticated user — each entry shows IP address, device/browser, and login time.',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Revokes all active sessions and refresh tokens for the authenticated user. Every device will require a fresh login.',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Closes a single active session by its ID. Useful for revoking access from a specific device without affecting others.',
  })
  @ApiResponse({
    status: 200,
  ...
  logoutSession(@CurrentUser('id') userId: string, @Param('sessionId') sessionId: string) {
